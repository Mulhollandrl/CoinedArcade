function highscoresPage (windowWidth, windowHeight, context){
    ...
    function processInput(keys) {
        ...
        if (keys.hasOwnProperty('Space') || keys.hasOwnProperty('Enter') || keys.hasOwnProperty('Escape')) {
            ...
            if (selectedButton === 1){
                ...
                localStorage.removeItem("Minigame.highscores")
            }
        }
    ...
    function render() {
        ...
        debugger
        previousScores = localStorage.getItem('Minigame.highscores')
        ...
        for (let highscoreIndex = 0; highscoreIndex < 5; highscoreIndex++) {
