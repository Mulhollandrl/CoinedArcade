function homePage (windowWidth, windowHeight, context, restartGame){
    // let audio = new Audio("assets/main.mp3");
    let aboutButton = button({y: 425, height: 50, text: "About!"}, windowWidth, context);
    let highscoresButton = button({y: 325, height: 50, text: "Highscores!"}, windowWidth, context);
    let newGameButton = button({y: 225, height: 50, text: "New Game!"}, windowWidth, context);
    ...
    function render() {
        let title = "Dodge-The-Tiles!";

        context.font = "48px Courier";

        let fontHeight = context.measureText("m").width;

        context.fillText(title, (windowWidth/2) - (context.measureText(title).width/2), 150 + (fontHeight/2));
